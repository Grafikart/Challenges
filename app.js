...
function nbr_parts(nbr_enfant=0, nbr_enfant_cmi=0, en_couple=false)
{
    ...
    return nbr_part + (.5*nbr_enfant) + (en_couple? 1 : 0) + (nbr_enfant_cmi/2);
}

function quotient_familial(revenu, nbr_enfant=0, nbr_enfant_cmi=0, en_couple=false)
{
    return revenu / nbr_parts(nbr_enfant, nbr_enfant_cmi, en_couple);
}

function montant_impot_du(revenu, nbr_enfant=0, nbr_enfant_cmi=0, en_couple=false)
{
    ...
    tranches.forEach( function(tranche, i){
        ...
        {
            data[`tranche_${i}`] = {
                montant_imposable: ((tranche.max - tranche.min+1) * tranche.bareme)/100
            }    
        }
...
let calcule_impot = function(e){
    ...
    if( !isNaN(revenu) )
    {
        let en_couple = parseInt(option_statut.value) === 1  || parseInt(option_statut.value) !== 0;
        let nbr_enfant = parseInt(option_enfant.value) || 0;
        let nbr_enfant_cmi = parseInt(option_enfant_cmi.value) || 0;
        let data = montant_impot_du(revenu, nbr_enfant=nbr_enfant, nbr_enfant_cmi=nbr_enfant_cmi, en_couple=en_couple);
        ...
        sections.forEach( (section, i) =>{
            ...
            let montant_impot_tranche = data[`tranche_${tmp_index_section}`].montant_imposable;
            section.querySelector(".tax_amount").textContent = (montant_impot_tranche).toLocaleString('fr') + " €";
        });
    }    
}
...
let impot = montant_impot_du(32000, nbr_enfant=0, nbr_enfant_cmi=0, en_couple=false);
console.log(impot);
impot = montant_impot_du(100000, nbr_enfant=0, nbr_enfant_cmi=0, en_couple=false);
