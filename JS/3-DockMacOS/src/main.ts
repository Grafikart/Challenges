...
class MacDock {
  ...
  constructor (el: HTMLElement, { position = Position.Left }: { position: Position }) {
    this.root = el
  ...
  scaleIcons () {
    ...
    const baseOffset = this.scaleFromDirection(selectedIndex, Direction.None, -centerOffset * this.iconSize * this.scale);
  ...
  scaleFromDirection (index: number, direction: Direction, offset: number): number {
    ...
    icon.style.setProperty('transform', `translate${this.isVertical ? 'Y' : 'X'}(${offset}px) scale(${scale + 1})`)
...
new MacDock(document.querySelector('.dock__wrapper--bottom .dock')!, { position: Position.Bottom })
new MacDock(document.querySelector('.dock__wrapper--top .dock')!, { position: Position.Top })
new MacDock(document.querySelector('.dock__wrapper--left .dock')!, { position: Position.Left })
new MacDock(document.querySelector('.dock__wrapper--right .dock')!, { position: Position.Right })
